...
class ConfigMagik{
	...
	*/
	function ConfigMagik( $synchronize=false, $process_sections=true, $path){
		 //$path=null,
		...
		if ( $path!=null) {
			...
			if ( !is_file( $path)) {
				...
				if ( !$fp_new) {
					$err = "ConfigMagik::ConfigMagik() - No se pudo crear el nuevo archivo de configuracion ('$path'), error.";
	...
	*/
	...
	function removeKey( $key, $section=null){
		...
		if ( $section!=null){
			if ( in_array( $section, array_keys( $this->VARS))==false){
				$err = "ConfigMagik::removeKey() - No se pudo encontrar la seccion('$section'), no se pudo remover.";
				array_push( $this->ERRORS, $err);
				return false;
			}
			// look if given key exists in given section
			if ( in_array( $key, array_keys( $this->VARS[$section]))===false) {
				$err = "ConfigMagik::removeKey() - No se pudo encontrar la clave('$key'), nada que remover.";
				array_push( $this->ERRORS, $err);
				return false;
			}
	...
	function removeSection( $section){
		...
		if ( in_array( $section, array_keys( $this->VARS), true)===false) {
			$err = "ConfigMagik::removeSection() - Seccion('$section') no se pudo encontrar, no se removio.";
			array_push( $this->ERRORS, $err);
			return false;
		}
	...
	*/
	function load( $path=null){
		...
		if ( $path!=null and !is_file( $path)) {
			$err = "ConfigMagik::load() - Ruta('$path') es invalido, no se pudo cargar.";
			array_push( $this->ERRORS, $err);
			echo $err;
			return false;
		}elseif ( $path==null){
	...
	*/
	function save( $path=null){
		...
		if ( !$handle = @fopen( $path, 'w')) {
			$err = "ConfigMagik::save() - No se pudo abrir el archivo ('$path') para escribir, error.";
			array_push( $this->ERRORS, $err);
			return false;
		}
		if ( !fwrite( $handle, $content)) {
			...
			$err = "ConfigMagik::save() - Archivo guardado exitosamente('$path').";
			array_push( $this->ERRORS, $err);
		}
		fclose( $handle);
		return true;
	...
	function listKeys( $section=null){
		...
		if ( $section!==null){
			...
			if ( in_array( $section, $sections)===false) {
				$err = "ConfigMagik::listKeys() - Seccion('$section') no se encontro.";
				array_push( $this->ERRORS, $err);
				return false;
			}
